...
// Расскладываем число N на М слагаемых, без нулей
// Например для (5, 3)
// 3	1	1
// 2	2	1
// 1	3	1
...
function numToTwoSummands($n, $m)
{
  ...
  if ($m == 2) { // при разложении на два слагаемых просто итерационно уменьшаем n на 1, а второе слагаемое увеличиваем
// Например для (4,2)
// 3	1
// 2	2
// 1	3
...
// Разложить число на множители (не только простые), т.е. получить представления числа
// Например 8 => [8, 2*4, 4*2, 2*2*2]
// @return array
function getMultipliers(int $n)
{
  $res = array($n);
  $i = $n-1;
  while ($i > 1) {
    if ($n % $i == 0) {
      $div = intdiv($n, $i);
      array_push($res, "$i * $div");
      if ($div > 3 && $div != $n) {
        $sub_res = getMultipliers($div);
        foreach ($sub_res as $re) {
          array_push($res, "$i * $re");
        }
      }
    }
...
// Вычисление N-го числа последовательности Фибоначи
